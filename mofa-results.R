...
p__factors_projection <- local({
  sample_df <- data.frame(Factor1 = numeric(318),
                          ...
                          Factor2 = numeric(318),
                          Subtype = rep("GBM", 318))
  sample_df$Factor1 <- factors[,1]
  sample_df$Factor3 <- factors[,3]
  sample_df$Factor2 <- factors[,2]
  sample_df$Subtype <- clinical$Subtype
...
ggsave(filename = "mofa-results/factors-projection.pdf", p__factors_projection, width = 6, height = 4)
